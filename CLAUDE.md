...
- **Pause/Resume:** Transient `isPaused` state in `App.tsx` (not part of `QuizState`). A hidden keyboard shortcut, **Ctrl+Shift+P**, toggles pause — there is no header button and the shortcut isn't shown anywhere in the UI. It only works while the quiz is active. Pausing stops the timer and shows a full-screen overlay (`.pause-overlay` in `index.css`) that hides questions. Pressing Ctrl+Shift+P again, or the overlay's Resume button, dismisses the overlay and continues the countdown. State resets on quiz restart.
